const GameAudio = {
    ...
    numToFiles(numStr) {
        ...
        const files = [];
        const len = s.length;
        const millionsPart = len > 6 ? parseInt(s.slice(0, -6)) : 0;
        const thousandsPart = len > 3 ? parseInt(s.slice(Math.max(0, len - 6), len - 3)) : 0;
        const unitsPart = parseInt(s.slice(-3));

        if (millionsPart > 0) {
            files.push(...this.chunkToFiles(millionsPart));
            files.push('milion');
        }
    ...
    getNaturalGap(fileName) {
        if (['va', 'yani', 'sadtayi', 'dehtayi', 'yeki'].includes(fileName)) {
            return 0.06;
        }
    ...
    speakMillionsIntro(milNum, onComplete) {
        const list = [
            'dar_tabaghe_milion_darim',
            ...this.chunkToFiles(milNum),
            'yani'
        ];
        this.playList(list, onComplete);
    },

    speakThousandsIntro(thNum, onComplete) {
        const list = thNum === 0 ? [
            'dar_tabaghe_hezar_adadi_nadarim',
            'yani'
        ] : [
            'dar_tabaghe_hezar_darim',
            ...this.chunkToFiles(thNum),
            'yani'
        ];
        this.playList(list, onComplete);
    },

    speakUnitsIntro(unNum, onComplete) {
        const list = unNum === 0 ? [
            'dar_tabaghe_yeki_adadi_nadarim',
            'yani'
        ] : [
            'dar_tabaghe_yeki_darim',
            ...this.chunkToFiles(unNum),
            'yani'
        ];
        this.playList(list, onComplete);
    },

    speakDigitPlace(digit, placeType, onComplete) {
        const list = [
            String(digit),
            placeType
        ];
    ...
    speakFinalNumber(numStr, onComplete) {
        const list = [
            'adad_shod',
            ...this.numToFiles(numStr)
        ];
        this.playList(list, onComplete);
    },

    speakAfarin(onComplete) {
        this.playList(['afarin'], onComplete);
    },
